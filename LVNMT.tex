...
\begin{figure}
	...
	\caption{Graphical models of \ac{LVNMT} models we consider. Left is the discriminative formulation and the Right is the joint or generative formulation.}
...
\end{equation}
These $h_{mean}$ vectors are passed through a single hidden layer \ac{MLP} that produces distribution parameters $\mu$ and $\log \sigma$ for a isotropic Gaussian distributionn. We use a \textit{softplus} operation on the $\log\sigma$ when sampling from the distribution. 
